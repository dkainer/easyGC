...
peak_parser.add_argument('-TS','--trimstart', required=True, default="[REQUIRED] 0.0", help='time in minutes (X.XX) in the chromatogram from where the analysis should begin. Helps to cut out junk at the start', type=str)
peak_parser.add_argument('-TE','--trimend', required=True, default="[REQUIRED] 20.0", help='time in minutes (X.XX) in the chromatogram where the analysis should end. Helps to cut out junk at the end', type=str)
...
peak_parser.add_argument('-M', '--noisemult', required=False, default=4.0, help='peak calling: total peak intensity must be at least this multiple of the base noise level to be called. Higher multiple means fewer peaks called', type=float)
...
pipe_parser.add_argument('-i', '--indir', required=True, help='[REQUIRED] absolute path to the directory containing your raw GC-MS files', type=str)
pipe_parser.add_argument('-f', '--ftype', required=True, choices=['CDF','JDX'], help='[REQUIRED] This is the type of input GC-MS files you have. CDF is not supported on Windows', type=str)
pipe_parser.add_argument('-TS','--trimstart', required=True, default=0.0, help='[REQUIRED] trim X.XX minutes from the start of the chromatogram', type=float)
pipe_parser.add_argument('-TE','--trimend', required=True, default=0.0, help='[REQUIRED] trim X.XX minutes from the end of the chromatogram', type=float)
...
pipe_parser.add_argument('-M', '--noisemult', required=False, default=4.0, help='peak calling: total peak intensity must be at least this multiple of the base noise level to be called. Higher multiple means fewer peaks called', type=float)
...
pipe_parser.add_argument('-T', '--threads', required=False, default=1, help='Peak calling is linearly sped up by using more threads. Currently only multithreaded on linux!', type=int)
