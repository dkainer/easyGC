...
from pyms.Peak.List.Function import mad_based_outlier, percentile_based_outlier
...
class Alignment(object):
    ...
    def write_csv_dk(self, rt_file_name, area_file_name, minutes=True):
        ...
        for peak_idx in range(len(self.peakpos[0])):
            ...
            #rts_np = rts_np[~numpy.isnan(rts_np)]
            #outliers = rts_np[mad_based_outlier(rts_np)]
            outliers = rts_np[percentile_based_outlier(rts_np)]
            ...
            avgrt = numpy.mean(rts_np[~percentile_based_outlier(rts_np)])
            print "outliers at RT "+str(avgrt)+":: "+str(outliers)

            # TO DO: exlcude outliers from compo-peak calculation
            compo_peak = composite_peak(new_peak_list[~percentile_based_outlier(rts_np)], minutes)
